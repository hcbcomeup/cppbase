...
void TestBasicUsage() {
  ...
  auto timer = std::make_shared<cppbase::Timer>(
      1000000000, []() { std::cout << "Timer callback executed!" << std::endl; });
...
void TestPeriodicTimer() {
  ...
  auto timer = std::make_shared<cppbase::Timer>(
      ...
      [&counter]() {
        counter++;
        std::cout << "Periodic timer executed, count: " << counter
                  << std::endl;
      },
      500000000);
...
void TestDelTimer() {
  ...
  std::cout << "Timer stopped successfully: "
            << (counter == oldCounter ? "yes" : "no") << std::endl;
  std::cout << std::endl;
}
...
void TestExceptionHandling() {
  ...
  auto timer = std::make_shared<cppbase::Timer>(
      100000000, []() { throw std::runtime_error("Test exception in timer"); });
...
void TestInvalidParam() {
  ...
  try {
    auto timer = std::make_shared<cppbase::Timer>(
        0, []() { std::cout << "Should not reach here" << std::endl; });
  ...
  try {
    ...
    auto timer =
        std::make_shared<cppbase::Timer>(1000000000, emptyFunc);
  } catch (const std::invalid_argument& e) {
    std::cout << "Caught exception for empty functor: " << e.what()
              << std::endl;
  }
