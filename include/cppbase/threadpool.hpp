...
public:
    ...
    int AddTask(Task &&task)
    {
        ...
        cvNotFull_.wait(lk, [this]
                     { return !running_ || !que_->IsFull(); });
    ...
    {
        std::unique_lock<std::mutex> lk(mutex_);
        auto status = cvNotFull_.wait_for(lk, waitTime, [this]
                     { return !running_ || !que_->IsFull(); });
...
private:
    void Run()
    {
        while (running_)
        {
            {
                std::unique_lock<std::mutex> lk(mutex_);
                cvNotEmpty_.wait(lk, [this]
                              { return !running_ || !que_->IsEmpty(); });
                if (!running_)
                {
                    break;
                }
