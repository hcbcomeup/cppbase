...
TEST_F(TimerMgrTest, AddTimer_Basic) {
  ...
  auto timer = std::make_shared<cppbase::Timer>(
      100000000, [&executed]() { executed = true; });
...
TEST_F(TimerMgrTest, PeriodicTimer) {
  ...
  auto timer = std::make_shared<cppbase::Timer>(
      100000000, [&counter]() { counter++; }, 100000000);
...
TEST_F(TimerMgrTest, DelTimer_NotFound) {
  ...
  auto timer = std::make_shared<cppbase::Timer>(1000000000, []() {});
...
TEST_F(TimerMgrTest, ExceptionHandling) {
  ...
  auto timer = std::make_shared<cppbase::Timer>(
      100000000, []() { throw std::runtime_error("Test exception"); });
...
TEST_F(TimerMgrTest, InvalidParam_Expiration) {
  EXPECT_THROW((void)std::make_shared<cppbase::Timer>(0, []() {}),
               std::invalid_argument);

  EXPECT_THROW((void)std::make_shared<cppbase::Timer>(-1000000000, []() {}),
               std::invalid_argument);
}

TEST_F(TimerMgrTest, InvalidParam_EmptyFunctor) {
  cppbase::Functor emptyFunc;
  EXPECT_THROW((void)std::make_shared<cppbase::Timer>(1000000000, emptyFunc),
               std::invalid_argument);
}

TEST_F(TimerMgrTest, TimerGetters) {
  cppbase::Timer timer(1000000000, []() {}, 500000000);
